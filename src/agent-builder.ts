...
import { buildSystemPrompt } from './prompts/template.js';
...
/**
 * Build cline-plan and cline-act agent configurations
 */
export function buildClineAgents(params: BuildAgentsParams): Record<string, AgentConfig> {
  const { config, defaultModel } = params;
  ...
  const actPrompt = buildSystemPrompt('act');
