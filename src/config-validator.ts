...
export function validateConfig(config: unknown): ValidationResult {
  ...
  if ('replace_default_agents' in cfg && typeof cfg.replace_default_agents !== 'boolean') {
    errors.push('replace_default_agents must be a boolean');
  }

  if ('default_agent' in cfg && !['cline-plan', 'cline-act'].includes(cfg.default_agent as string)) {
